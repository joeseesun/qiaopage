...
function objectStorage(dbPath, env = process.env) {
  ...
  if (backend === "vercel-blob") return new (require("./blob").BlobObjects)(env.BLOB_PREFIX);
  if (backend === "s3") {
  ...
  if (backend !== "filesystem")
    throw new Error("Unsupported OBJECT_STORE; choose filesystem, s3 or vercel-blob");
  if (dbPath === ":memory:") {
